...
| **A2** | Inline `Value to me: AED X` after every H3 benefit + summing conclusion | **NOT STARTED** | `grep -c "Value to me" src/content/cards/*.mdx` returns **0**. Zero cards carry the pattern. HfP signature pattern; pure MDX convention, no schema change required |
...
| **B11** | "As seen in" press-logo strip | **NOT STARTED** | 0 hits across `src/pages/index.astro` and `src/components/`. Teardown explicitly notes this is a "dormant slot until we have placements" |
