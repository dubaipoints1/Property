// Emirates NBD scraper entry point.
//
// Reads scripts/scrape/banks/enbd.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/enbd/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:enbd
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:enbd
//
// Companion notes / open data gaps: scripts/scrape/banks/enbd.notes.md.
...
const BANK_SLUG = "enbd";
const URLS_PATH = path.join("scripts", "scrape", "banks", "enbd.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[enbd] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[enbd] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[enbd] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[enbd] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[enbd]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[enbd] Fatal:", err);
  process.exit(1);
});
