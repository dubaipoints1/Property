// RAKBANK scraper entry point.
//
// Reads scripts/scrape/banks/rakbank.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/rakbank/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:rakbank
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:rakbank
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/rakbank.notes.md.
...
const BANK_SLUG = "rakbank";
const URLS_PATH = path.join("scripts", "scrape", "banks", "rakbank.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[rakbank] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[rakbank] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[rakbank] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[rakbank] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[rakbank]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[rakbank] Fatal:", err);
  process.exit(1);
});
