// Wio Bank scraper entry point.
//
// Reads scripts/scrape/banks/wio.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/wio/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:wio
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:wio
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/wio.notes.md.
...
const BANK_SLUG = "wio";
const URLS_PATH = path.join("scripts", "scrape", "banks", "wio.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[wio] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[wio] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[wio] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[wio] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[wio]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[wio] Fatal:", err);
  process.exit(1);
});
