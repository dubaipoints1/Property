// HSBC scraper entry point.
//
// Reads scripts/scrape/banks/hsbc.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/hsbc/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:hsbc
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:hsbc
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/hsbc.notes.md.
...
const URLS_PATH = path.join("scripts", "scrape", "banks", "hsbc.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[hsbc] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[hsbc] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[hsbc] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[hsbc] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[hsbc]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[hsbc] Fatal:", err);
  process.exit(1);
});
