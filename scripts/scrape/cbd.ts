// CBD scraper entry point.
//
// Reads scripts/scrape/banks/cbd.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/cbd/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:cbd
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:cbd
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/cbd.notes.md.
...
const BANK_SLUG = "cbd";
const URLS_PATH = path.join("scripts", "scrape", "banks", "cbd.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[cbd] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[cbd] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[cbd] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[cbd] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[cbd]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[cbd] Fatal:", err);
  process.exit(1);
});
