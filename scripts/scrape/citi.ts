// CITI scraper entry point.
//
// Reads scripts/scrape/banks/citi.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/citi/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:citi
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:citi
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/citi.notes.md.
...
const URLS_PATH = path.join("scripts", "scrape", "banks", "citi.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[citi] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[citi] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[citi] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[citi] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[citi]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[citi] Fatal:", err);
  process.exit(1);
});
