// ADIB scraper entry point.
//
// Reads scripts/scrape/banks/adib.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/adib/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:adib
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:adib
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/adib.notes.md.
...
const BANK_SLUG = "adib";
const URLS_PATH = path.join("scripts", "scrape", "banks", "adib.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[adib] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[adib] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[adib] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[adib] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[adib]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[adib] Fatal:", err);
  process.exit(1);
});
