// MASHREQ scraper entry point.
//
// Reads scripts/scrape/banks/mashreq.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/mashreq/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:mashreq
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:mashreq
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/mashreq.notes.md.
...
const BANK_SLUG = "mashreq";
const URLS_PATH = path.join("scripts", "scrape", "banks", "mashreq.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[mashreq] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[mashreq] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[mashreq] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[mashreq] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[mashreq]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[mashreq] Fatal:", err);
  process.exit(1);
});
