// DIB scraper entry point.
//
// Reads scripts/scrape/banks/dib.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/dib/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:dib
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:dib
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/dib.notes.md.
...
const BANK_SLUG = "dib";
const URLS_PATH = path.join("scripts", "scrape", "banks", "dib.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[dib] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[dib] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[dib] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[dib] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[dib]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[dib] Fatal:", err);
  process.exit(1);
});
