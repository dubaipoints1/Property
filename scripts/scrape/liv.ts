// Liv (Emirates NBD digital brand) scraper entry point.
//
// Reads scripts/scrape/banks/liv.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/liv/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:liv
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:liv
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/liv.notes.md.
...
const BANK_SLUG = "liv";
const URLS_PATH = path.join("scripts", "scrape", "banks", "liv.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[liv] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[liv] Scraping ${cards.length} card(s)`);
  ...
  console.log(`[liv] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[liv] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[liv]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[liv] Fatal:", err);
  process.exit(1);
});
