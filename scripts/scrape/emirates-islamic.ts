// EMIRATES-ISLAMIC scraper entry point.
//
// Reads scripts/scrape/banks/emirates-islamic.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/emirates-islamic/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:emirates-islamic
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:emirates-islamic
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/emirates-islamic.notes.md.
...
const BANK_SLUG = "emirates-islamic";
const URLS_PATH = path.join("scripts", "scrape", "banks", "emirates-islamic.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[emirates-islamic] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[emirates-islamic] Scraping ${cards.length} card(s)`);
  ...
  for (const card of cards) {
    ...
    for (const url of urls) {
      console.log(`[emirates-islamic] Fetching ${url}`);
      const fetched = await firecrawlFetch(url);
      sources.push(fetched);
      if (fetched.status === "fail") {
        console.warn(`[emirates-islamic]   ✗ ${fetched.failReason}`);
      }
    }
  ...
  console.log(`[emirates-islamic] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[emirates-islamic] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[emirates-islamic]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[emirates-islamic] Fatal:", err);
  process.exit(1);
});
