// STANDARD-CHARTERED scraper entry point.
//
// Reads scripts/scrape/banks/standard-chartered.urls.json, fetches each card's URL set via
// Firecrawl, runs the normaliser, and writes a JSON snapshot to
// data/scraped/standard-chartered/<timestamp>.json.
//
// Run:
//   FIRECRAWL_API_KEY=fc-... npm run scrape:standard-chartered
//
// Dry run (no API call, validates wiring):
//   FIRECRAWL_API_KEY=skip npm run scrape:standard-chartered
//
// Companion notes / editor-confirmation gaps: scripts/scrape/banks/standard-chartered.notes.md.
...
const URLS_PATH = path.join("scripts", "scrape", "banks", "standard-chartered.urls.json");
...
async function main() {
  if (!fs.existsSync(URLS_PATH)) {
    console.error(`[standard-chartered] URL config missing: ${URLS_PATH}`);
    process.exit(1);
  }
  const cards = JSON.parse(fs.readFileSync(URLS_PATH, "utf8")) as CardUrlSet[];
  console.log(`[standard-chartered] Scraping ${cards.length} card(s)`);
  ...
  for (const card of cards) {
    ...
    for (const url of urls) {
      console.log(`[standard-chartered] Fetching ${url}`);
      const fetched = await firecrawlFetch(url);
      sources.push(fetched);
      if (fetched.status === "fail") {
        console.warn(`[standard-chartered]   ✗ ${fetched.failReason}`);
      }
    }
  ...
  console.log(`[standard-chartered] Wrote ${file}`);

  const failures = results.filter((r) => r.errors.length > 0);
  if (failures.length > 0) {
    console.log(`[standard-chartered] ${failures.length} card(s) had parsing warnings:`);
    for (const f of failures) {
      console.log(`[standard-chartered]   ${f.slug}: ${f.errors.join("; ")}`);
    }
  }
}

main().catch((err) => {
  console.error("[standard-chartered] Fatal:", err);
  process.exit(1);
});
